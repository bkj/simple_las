...
class simpleAS():
  ...
  def __init__ (self, X, init_labels, params=Parameters()):
    # Init params
    self.params = params
    self.l = (1 - self.params.eta) / self.params.eta
    if self.params.w0 is None:
      self.params.w0 = 1 / n_obs
  ...
  def _nominate_next_message(self):
    next_idx = np.argmax((self.f + self.params.alpha * self.IM)[self.unlabeled_idxs])
  ...
  def _init_matrices(self):
    ...
    B          = np.where(self.labels == -1, 1 / (1 + self.params.w0), self.l / (1 + self.l))
    y_prime    = np.where(self.labels == -1, self.params.pi, self.labels)
    ...
    if self.params.alpha > 0:
      self.dP = (1. / self.l - self.params.w0) * D
      self.dPpi = (1. / self.l - self.params.pi * self.params.w0) * D # L - pi*U
      self.z = np.where(self.labels == -1, self.BDinv, 0)
  ...
  def _update_matrices(self, idx, lbl):
    ...
    if self.params.alpha > 0:
      self.z[idx] = 0
      self.J = self.J - c * (np.squeeze(self.X.T.dot(Kinv_Xi)) ** 2)
      self.IM = self._compute_IM()
